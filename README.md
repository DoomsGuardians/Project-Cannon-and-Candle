...
| 系统 | 说明 |
|------|------|
| 现货交易 | 买卖 ATK/DEF/RET 三种军需指令，价格受战局供需驱动 |
| 期货市场 | 20% 保证金杠杆交易，支持做多/做空 |
| 银行借贷 | 按净资产倍数借款，每回合计息 |
| 三因子定价 | Alpha(战线) + Beta(供需) + Gamma(产能) 驱动价格 |
| 战斗系统 | 三线战场(左翼/中军/右翼)，ATK/DEF/RET 剪刀石头布 |
| 将军系统 | 三种性格(狂热/保守/投机)，12个技能，意图强化/篡改 |
| 委托任务 | 多条件奖金任务，增加策略深度 |
...
- **UI 窗口管理**: 完整的 UI 生命周期和事件绑定（WindowBase 为纯 C# 类，Binder 模式序列化引用）
- **状态机系统**: 通用的有限状态机实现
- **对象池**: 内置对象池支持
